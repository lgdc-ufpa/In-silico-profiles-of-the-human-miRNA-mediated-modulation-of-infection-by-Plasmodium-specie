...
dataframe = calc_dataframe()
# dataframe.pivot('Species', 'micro')
sns.set(rc={'figure.figsize':(11.7, 8.27)})
# sns.set(rc={'figure.figsize':(11.7, 8.27*4)})

sns.set(font_scale=1.4)
color_bar_legend = 'Number of times that a micro-RNA was predicted.'
# plot_heatmap = sns.heatmap(data=dataframe.T,
#                            # cbar_kws={'label': color_bar_legend},
#                            cbar=False,
#                            annot=True,
#                            fmt='.02f'
#                            )
maximo = float(dataframe.to_numpy(dtype=float).max().max())
print(maximo)
...
# colors = [c3, c2, c1, c4, c7, c6, c5]
# print(colors)

plot_heatmap = sns.heatmap(data=dataframe,
                           cbar_kws={'label': color_bar_legend},
                           xticklabels=False,
                           # vmax=maximo,
                           # annot=True,
...
plt.savefig(f'{path}', dpi=500, bbox_inches='tight')
plt.show()
plt.clf()
plt.close()
